class site24x7::linuxagent {
  ...
  if inline_template("<%= `/usr/bin/test -d /opt/site24x7/monagent && echo 'Yes' || echo 'No'` %>") == "No\n"{
    ...
    if $customer_id != "" {
      ...
      if $agent_proxy != "" {
        $command = "/opt/.site24x7agent/./Linux_Agent.install -i -key=${customer_id} -installer=puppet  -proxy=${agent_proxy}"
        include site24x7::installation_handler
      }
      if $agent_proxy == "" {
        $command = "/opt/.site24x7agent/./Linux_Agent.install -i -key=${customer_id} -installer=puppet"
        include site24x7::installation_handler
      }
      notify {
        "Site24x7 Linux Agent installation in progress":
      } ->
      notify {
        "File To Download : ${install_file}":
      } ->
